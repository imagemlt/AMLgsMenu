/*
	...
	Adapted from the MIT-licensed ImGui-Terminal project (Neal 2025).
*/
#include "terminal.h"
...
{
	static bool logged_visible = false;
    if (!isVisible) {
        logged_visible = false;
        return;
    }
    bool font_pushed = false;
    if (font_override_) {
        ImGui::PushFont(font_override_);
        font_pushed = true;
    }
								  ...
								  float &lineHeight)
{
	...
	charWidth = ImGui::CalcTextSize("M").x;
							   ...
							   float lineHeight)
{
	...
	if (ImGui::IsWindowFocused())
	{
		...
		float alpha = (std::sin(ImGui::GetTime() * 3.14159f) * 0.3f) + 0.5f;
