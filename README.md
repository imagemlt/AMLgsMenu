...
## Features
- OSD mock (or MAVLink) data: horizon, signals (dBm), flight mode, GPS + home distance, video bitrate/resolution/refresh, battery, sky/ground temps. In WFB mode, ground antenna readings and bitrate parse the latest `RX_ANT`/`PKT` entries from the `wifibroadcast` journal; in `ap_mode=1`, signal and bitrate switch to `iw dev <wlan> link` (RSSI plus WiFi link bitrate). The top-left label shows `%d WIFI PLUGGED` based on monitor-mode interfaces from `iw dev` (real data only).
...
- UDP config push (fire-and-forget) to 127.0.0.1:14650/14651: channel, bandwidth, sky mode (size/fps, restart majestic), bitrate (Mbps→kbps), sky power (p*50 mBm). Ground channel applies to local monitor interfaces via `iw` (with HT20/HT40+ suffix); ground power targets monitor interfaces in WFB mode and the configured `wlan` interface when `ap_mode=1`.
